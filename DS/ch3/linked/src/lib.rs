...
pub struct LinkedList<T> {
    ...
    back: Link<T>,
    len: usize,
    // 被用来表示 链表用于 T 类型的元素。用来告诉 编译器 这个链表 与 T 类型相关
    _boo: PhantomData<T>,
}
...
// cursorMut 与 iterMut 的区别： 不能通过 IterMut 增加一个节点
    ...
    pub fn splice_before(&mut self, mut input: LinkedList<T>) {
        ...
        unsafe {
            ...
            if input.is_empty() {
                ...
                let in_front = input.front.take().unwrap(); // 从 input 链表中 拿走 front 的所有权
